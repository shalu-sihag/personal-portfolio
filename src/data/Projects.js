const projects = [
  {
    title: "InternFlow",
    description:
      "A job and internship tracker for managing applications, interviews, and statuses.",
    tech: ["React", "JavaScript", "CSS"],
    github: "https://github.com/shalu-sihag/internflow",
    live: "https://shalu-sihag.github.io/internflow/"
  },

  {
    title: "AI Study Planner",
    description:
      "An AI-powered study planner that helps students organize tasks and improve productivity.",
    tech: ["React", "API", "JavaScript"],
    github: "https://github.com/shalu-sihag/ai-study-planner"
  },

  {
    title: "Currency Converter",
    description:
      "Real-time currency conversion using live API integration.",
    tech: ["React", "API", "JavaScript"],
    github: "https://github.com/shalu-sihag/currency-converter",
    live: "https://shalu-sihag.github.io/currency-converter/"
  },

  {
    title: "Solar System Animation",
    description:
      "Animated solar system simulation built using HTML, CSS, and JavaScript.",
    tech: ["HTML", "CSS", "JavaScript"],
    github: "https://github.com/shalu-sihag/solar-system-animation",
    live: "https://shalu-sihag.github.io/solar-system-animation/"
  },

  {
    title: "Rock Paper Scissors",
    description:
      "Classic game project with score tracking and game logic.",
    tech: ["HTML", "CSS", "JavaScript"],
    github: "https://github.com/shalu-sihag/rock-paper-scissor-game"
  }
];

export default projects;